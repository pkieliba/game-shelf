...
function ParseInput(file) {
    Papa.parse(file, {header: true, complete: function(results, file){
        console.log('Input parsed correctly'); gameLibrary = results.data; filterData(gameLibrary);
    }});
}
...
function filterData(library) {
    ...
    if (gameType.coop == false && gameType.compete == false) {
        ...
        document.querySelector("#reroll-button").style.visibility= "hidden";
        ...
        document.querySelector(".chosen").style.display= "flex";
...
function printGame(item){
    ...
    document.querySelector(".filtered-pics").innerHTML +=
    "<div class='col games-box'><div id=" + itemName + "_container class='filtered-container' data-toggle='tooltip' data-container='body' data-placement='right' data-html='true'" +
    ...
    itemName + " checked /><label class='game-lab' for=" + itemName + "></label><span>" + item + "</span></div><p class='game-name'>" + item + "</p></div>";
    /* change background of the container to game box image */
    var imageBox = new Image();
    imageBox.onload = function() {
        document.getElementById(itemName + "_container").style.background = "url(img/gameBoxes/" + itemName.replace(/[^\w\s]|_/g, "") + ".jpg) 50% 75%/ cover";
        document.getElementById(itemName + "_container").getElementsByTagName("span")[0].innerHTML = "";
    };
    imageBox.src = "img/gameBoxes/" + itemName.replace(/[^\w\s]|_/g, "") + ".jpg";
...
function chooseRandom(games){
    ...
    var chosenName = chosenGame.replace(/ /g, '');
    document.querySelector("#chosen-game").innerHTML = "<div class='col games-box'><div class='chosen-container'>" + chosenGame + "</div><p class='chosen-name'>" + chosenGame + "</p></div>";
    var chosenBox = new Image();
    chosenBox.onload = function() {
        document.querySelector(".chosen-container:first-child").style.background = "url(img/gameBoxes/" + chosenName.replace(/[^\w\s]|_/g, "") + ".jpg) 50% 75%/ cover";
        document.querySelector(".chosen-container:first-child").innerHTML = "";
    };
    chosenBox.src = "img/gameBoxes/" + chosenName.replace(/[^\w\s]|_/g, "") + ".jpg";
